...
const greeting = document.querySelector("#greeting h1"); // h1 태그 선택
...
function paintGreeting(username) {
  ...
  document.querySelector("main").classList.remove(HIDDEN_CLASSNAME);
}

function onLoginSubmit(event) {
  event.preventDefault(); // 이벤트 발생 시 기본 동작(새로고침) 방지
  loginWrapper.classList.add(HIDDEN_CLASSNAME); // submit 이벤트 발생 시 hidden 클래스 추가
...
if (savedUsername === null) {
  ...
  loginWrapper.classList.remove(HIDDEN_CLASSNAME); // hidden 클래스 제거
